...
const Palette: React.FC = () => {
  ...
  const [draggedBoxData, setDraggedBoxData] = useState<{
    x: number;
    y: number;
    box: Box;
  } | null>(null);
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (canvasBounds && draggedBoxData) {
      ...
      if (box.r === 0 || box.r === 180) {
        newX = Math.max(
          boxSize.width / 2,
          Math.min(newX, paletteSize.width - boxSize.width / 2)
        );
        newY = Math.max(
          boxSize.height / 2,
          Math.min(newY, paletteSize.height - boxSize.height / 2)
        );
      }
      // If the box has 90 or 270-degree rotation
        ...
        newX = Math.max(
          boxSize.height / 2,
          Math.min(newX, paletteSize.width - boxSize.height / 2)
        );
        newY = Math.max(
          boxSize.width / 2,
          Math.min(newY, paletteSize.height - boxSize.width / 2)
        );
      }
      ...
      if (collidedBoxes.length) {
        ...
        collidedBoxes.sort((a, b) => {
          return (
            Math.sqrt(Math.pow(a.x - newX, 2) + Math.pow(a.y - adjustedY, 2)) -
            Math.sqrt(Math.pow(b.x - newX, 2) + Math.pow(b.y - adjustedY, 2))
          );
        });
  ...
  const handleRotationUpdate = (box: Box) => {
    ...
    if (payload.r === 0 || payload.r === 180) {
      payload.x = Math.max(
        boxSize.width / 2,
        Math.min(payload.x, paletteSize.width - boxSize.width / 2)
      );
      payload.y = Math.max(
        boxSize.height / 2,
        Math.min(payload.y, paletteSize.height - boxSize.height / 2)
      );
    } else if (payload.r === 90 || payload.r === 270) {
      payload.x = Math.max(
        boxSize.height / 2,
        Math.min(payload.x, paletteSize.width - boxSize.height / 2)
      );
      payload.y = Math.max(
        boxSize.width / 2,
        Math.min(payload.y, paletteSize.height - boxSize.width / 2)
      );
    }
